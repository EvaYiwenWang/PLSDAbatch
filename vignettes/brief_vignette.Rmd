...
This vignette only includes the necessary pre-processing steps (pre-filtering and transformation) for microbiome data and the usage of PLSDA-batch series methods. See "[Batch Effects Management in Case Studies](https://evayiwenwang.github.io/PLSDAbatch_workflow/articles/case_studies.html)" for the complete batch effect management process including data pre-processing, batch effect detection and visualisation, choices of methods to account for or correct for batch effects, assessment of batch effect removal and variable selection after batch effect correction.
...
# load packages 
sapply(bioc.pkgs, require, character.only = TRUE)
